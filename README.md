...
Use `--traffic-rule-guard` when you want CARLA to assist braking for red/yellow lights and stop signs. The guard can creep a stopped car closer to a red-light trigger line, release leftover braking when the light turns green, and cautiously unstick model-only false stops when the car is centered, straight, and no close obstacle is detected. In junctions, that release uses an even gentler creep. Leave it off for a pure image-only model evaluation.
